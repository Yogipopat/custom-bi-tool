...
# --- Mode Selection ---
mode = st.radio("Select Mode", ["Single File", "Compare Two Files"], horizontal=True)

if mode == "Compare Two Files":
    col_a, col_b = st.columns(2)
    with col_a:
        file_a = st.file_uploader("Upload File 1", type=["xlsx", "xls"], key="file_a")
    with col_b:
        file_b = st.file_uploader("Upload File 2", type=["xlsx", "xls"], key="file_b")

    if file_a and file_b:
        df_a = pd.read_excel(file_a, sheet_name=0)
        df_b = pd.read_excel(file_b, sheet_name=0)

        label_a = file_a.name.replace(".xlsx", "").replace(".xls", "")
        label_b = file_b.name.replace(".xlsx", "").replace(".xls", "")

        num_cols_a = df_a.select_dtypes(include='number').columns.tolist()
        num_cols_b = df_b.select_dtypes(include='number').columns.tolist()
        common_num = [c for c in num_cols_a if c in num_cols_b]
        common_cat = [c for c in df_a.select_dtypes(exclude='number').columns
                      if c in df_b.columns and not pd.api.types.is_datetime64_any_dtype(df_a[c])]

        # --- Common Filters (apply to both files) ---
        st.sidebar.header("Filters (Both Files)")
        for col in common_cat[:3]:
            all_options = sorted(set(df_a[col].dropna().unique().tolist() + df_b[col].dropna().unique().tolist()))
            selected = st.sidebar.multiselect(f"{col}", all_options, default=all_options, key=f"comp_filter_{col}")
            df_a = df_a[df_a[col].isin(selected)]
            df_b = df_b[df_b[col].isin(selected)]

        st.sidebar.markdown(f"**{label_a}: {len(df_a)} rows**")
        st.sidebar.markdown(f"**{label_b}: {len(df_b)} rows**")
        ...
        for col in common_num:
            ...
            m1.metric(f"{col} — {label_a}", f"{val_a:,.0f}")
            m2.metric(f"{col} — {label_b}", f"{val_b:,.0f}", f"{diff_pct:+.1f}%")
            m3.metric("Difference", f"{diff:+,.0f}")
        ...
        with comp_col1:
            comp_x = st.selectbox("X-axis", common_cat, key="comp_x")
        with comp_col2:
            comp_y = st.selectbox("Y-axis", common_num, key="comp_y")

        grp_a = df_a.groupby(comp_x)[comp_y].sum().reset_index()
        grp_a["File"] = label_a
        grp_b = df_b.groupby(comp_x)[comp_y].sum().reset_index()
        grp_b["File"] = label_b

        combined = pd.concat([grp_a, grp_b])
        fig_comp = px.bar(
            combined, x=comp_x, y=comp_y,
            color="File",
            barmode="group",
            color_discrete_map={label_a: "#F0A500", label_b: "#185FA5"},
        )
        ...
        tab1, tab2 = st.tabs([label_a, label_b])
        with tab1:
        ...
        with dl1:
            st.download_button(f"⬇️ Download {label_a}", csv_a, f"{label_a}.csv", "text/csv", key="dl_a")
        with dl2:
            st.download_button(f"⬇️ Download {label_b}", csv_b, f"{label_b}.csv", "text/csv", key="dl_b")
    ...
    if uploaded_file:
        ...
        with col1:
            ...
            chart_type = st.selectbox("Chart type", ["Bar", "Line", "Pie", "Scatter", "Area", "Donut", "Box Plot", "Funnel", "Treemap", "Gauge", "Heatmap"])
            ...
            if chart_type == "Bar":
                ...
                total_val = filtered_df[y_axis].sum()
                max_val = total_val * 1.5
                fig = go.Figure(go.Indicator(
                    mode="gauge+number+delta",
                    value=total_val,
                    title={"text": f"{y_axis} Total"},
                    delta={"reference": kpi_target if kpi_col == y_axis else total_val * 0.8},
                    gauge={
                        "axis": {"range": [0, max_val]},
                        "bar": {"color": "#F0A500"},
                        "steps": [
                            {"range": [0, max_val * 0.5], "color": "#2C3E50"},
                            {"range": [max_val * 0.5, max_val * 0.8], "color": "#1E2130"},
                            {"range": [max_val * 0.8, max_val], "color": "#27500A"},
                        ],
                        "threshold": {
                            "line": {"color": "red", "width": 4},
                            "thickness": 0.75,
                            "value": kpi_target if kpi_col == y_axis else max_val * 0.8
                        }
                    }
                ))
                ...
                heatmap_data = filtered_df.pivot_table(
                    index=x_axis,
                    columns=cat_cols[1] if len(cat_cols) > 1 else x_axis,
                    values=y_axis,
                    aggfunc="sum",
                    fill_value=0
                )
